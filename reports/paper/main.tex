...
data. Three findings emerge. First, the measurement thesis is validated: every score- and
...
disclosure is associated with 8.7 percentage points higher year-over-year
...
($p=0.03$)---precisely where AI narratives vary most in credibility and
structured measurement is most needed---and that AI-infrastructure suppliers
outperformed sector- and size-matched peers by 37 percentage points over the
...
its full-specification coefficient of \coef{0.087} \pval{0.007} implies that
moving from the bottom to the top of the concreteness distribution is
associated with 8.7 percentage points higher revenue growth, holding sector,
...
Investment intensity sits in between (marginally significant). The practical
...
approximately 37 percentage points over four months---a striking decomposition
...
group ($n=214$, $p=0.029$): among companies where AI claims are cheapest to
...
resolve. Where one company lists multiple share classes, one line is retained.
...
membership, firm size, and growth persistence. Each estimate uses all firms
...
also report the correction across the full $7\times3$ signal--outcome matrix.
...
\coef{0.087} \pval{0.007} implies an 8.7-point revenue-growth gap between the
...
the primary family; investment intensity is directionally positive but
marginal \pval{0.10}.
...
\midrule
Narrative concreteness (ours) & 0.148*** & 0.094*** & 0.084** & \textbf{0.087***} & 399\\
...
\emph{Physical-Asset-Heavy / Late Adopters} (\coef{0.064}, \pval{0.029},
$n=214$) and within \emph{AI Software \& Platforms} (\coef{0.074}, $p=0.050$), while composite scores are insignificant within every category.
...
suppliers returned $+48.9\%$ on average over four months, versus roughly flat
for every other category, and the gap survives sector and size controls at
$+36.8$ percentage points ($p<0.0001$). We present this as a descriptive
decomposition of the H1-2026 AI rally---the classification is made with
knowledge of these firms---rather than as an ex-ante trading result; even so,
it quantifies how narrowly concentrated AI's market-value creation has been.
...
\midrule
AI Infrastructure \& Hardware & $+0.146$ (0.143) & 26 & $+48.9\%$ & 30\\
AI Software \& Platforms & $+0.074$ (0.050) & 35 & $-3.8\%$ & 38\\
Data-Rich Adopters & $+0.093$ (0.198) & 101 & $-3.5\%$ & 132\\
Physical-Asset-Heavy / Late & $+0.064$ (0.029) & 214 & $+1.8\%$ & 278\\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
date. The infrastructure-vs-late-adopter return gap is $+36.8$pp with sector
and size controls ($p<0.0001$).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
(IC $=+0.15$, $p=0.002$; Appendix Table~\ref{tab:ic}), and passes
primary-family FDR control; across the full fifteen-test matrix it clears
$q=0.10$ but not $q=0.05$, which we report plainly. In the strict
complete-case subsample---firms with every signal, outcome, and control
observed, which halves the sample to 199---the coefficient is essentially
unchanged (\coef{0.076} vs.\ \coef{0.087}) with precision reduced accordingly
($p=0.11$; Appendix Table~\ref{tab:cc}), consistent with a loss of power
rather than sensitivity to sample composition. Score-bin sorts show the
association is monotone (Appendix Table~\ref{tab:sorts}). Four further checks
support the result: a placebo signal (company-name length) run through the
identical pipeline is null ($p=0.70$); in a permutation test, the observed
concreteness coefficient exceeds all 500 label-shuffled re-estimates
(permutation $p<0.002$); the coefficient remains significant when any single
sector is excluded (worst case: dropping technology, \coef{0.073}, $p=0.030$);
and re-scoring a random 30-company subsample yields 93\% exact score agreement
(100\% within one point; concreteness rerun rank correlation $0.94$),
indicating the LLM scoring itself is stable. Evidence citation audits,
...
\midrule
Narrative concreteness & $+0.236$*** & $+0.032$ & $+0.001$ & $+0.153$***\\
 & (401) & (319) & (461) & \\
Investment intensity & $+0.164$*** & $+0.027$ & $+0.003$ & $+0.062$\\
 & (389) & (310) & (446) & \\
Larridin adoption & $+0.189$*** & $+0.045$ & $-0.059$ & $+0.116$**\\
 & (435) & (346) & (502) & \\
Larridin maturity index & $+0.184$*** & $+0.068$ & $-0.053$ & $+0.107$**\\
 & (435) & (346) & (502) & \\
Larridin impact & $+0.177$*** & $+0.065$ & $-0.042$ & $+0.111$**\\
 & (435) & (346) & (502) & \\
Larridin proficiency & $+0.126$*** & $+0.047$ & $-0.056$ & $+0.039$\\
 & (435) & (346) & (502) & \\
AI-hiring builder rate & $+0.012$ & $+0.038$ & $+0.027$ & $-0.076$\\
 & (216) & (177) & (251) & \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
\midrule
1 & $-9.4\%$ & 2 & Q1 (low) & $8.5\%$ & 88\\
2 & $7.3\%$ & 100 & Q2 & $8.1\%$ & 91\\
3 & $9.1\%$ & 141 & Q3 & $11.6\%$ & 85\\
4 & $13.5\%$ & 151 & Q4 & $11.0\%$ & 81\\
5 & $52.2\%$ & 7 & Q5 (high) & $16.6\%$ & 90\\
\midrule
\multicolumn{3}{l}{High (4--5) $-$ low (1--2): $+8.2$pp ($t=4.18$)} &
\multicolumn{3}{l}{Q5 $-$ Q1: $+8.1$pp}\\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
...
return sorts show no monotone pattern (Q1 $+2.6\%$ to Q5 $-0.7\%$).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tabular}{lcc}
\toprule
Signal & Coefficient ($p$) & Full-sample coefficient ($p$)\\
\midrule
Narrative concreteness & $+0.076$ (0.110) & $+0.087$ (0.007)\\
Investment intensity & $+0.000$ (0.994) & $+0.048$ (0.100)\\
Larridin adoption & $+0.026$ (0.716) & $+0.037$ (0.235)\\
Larridin maturity index & $+0.018$ (0.769) & $+0.024$ (0.427)\\
AI-hiring builder rate & $-0.033$ (0.154) & $-0.033$ (0.108)\\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
\item The complete-case subsample ($n=199$) requires every signal, the primary
...
prior revenue growth, HC3 standard errors. The concreteness coefficient is
essentially unchanged while precision falls with the halved sample.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\centering
\begin{threeparttable}
\caption{Variable coverage in the analysis sample ($n=518$ listed companies)}
\label{tab:coverage}
\small
\begin{tabular}{lcl}
\toprule
...
\midrule
Larridin maturity / adoption & 514 & four firms unscored in vintage\\
Narrative concreteness / investment & 462 / 447 & no U.S.\ 10-K (foreign filers); no AI content\\
AI-hiring builder rate & 253 & fewer than five matched postings\\
Revenue growth (YoY) & 438 & non-comparable revenue concepts (banks)\\
Operating-margin change (YoY) & 348 & banks and insurers\\
Four-month forward return & 506 & delistings\\
Log market capitalization & 508 & missing shares data\\
Prior revenue growth & 500 & insufficient reported history\\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
